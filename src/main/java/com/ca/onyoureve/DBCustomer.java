        ...
        try {
            ...
            if (conn != null) {
                ...
                while (rs.next()) {                                            // reads a row
                    ...
                    Customer cust = new Customer(name, nid, age, phone, email, acc, ps);
                    return cust;
                }
            } else {
                System.out.println("Failed to make connection!");
    ...
    public void insertCustomer() {
        ...
        try {
            ...
            if (conn != null) {
                ...
                query = "insert into customer(CUST_ID, name, nid, age, phone_no, email, ACCOUNT_NUMBER, PRIORITY_STATUS)" +
                        "values('" + new_id + "','" + cust.getName() + "','" + cust.getnid() + "'," + Integer.toString(cust.getAge()) + ",'" + cust.getContact_info() +
    ...
    public void insertCustomer(Customer cust, String id) {
        try {
            ...
            if (conn != null) {
                ...
                Statement stmt = conn.createStatement();
                String query = "insert into customer(CUST_ID, name, nid, age, phone_no, email, ACCOUNT_NUMBER, PRIORITY_STATUS)" +
                        "values('" + id + "','" + cust.getName() + "','" + cust.getnid() + "'," + Integer.toString(cust.getAge()) + ",'" + cust.getContact_info() +
                        "','" + cust.getEmail_address() + "','" + cust.getAccount_number() + "'," + Integer.toString(cust.getPriority_status()) + ")";
                //System.out.println(query);
                stmt.executeUpdate(query);
                stmt.executeUpdate("commit");
