...
public class EditCustomerController {
    ...
    public void init() {
        ...
        account.setText(cust.getAcc_no());
    ...
    public void handleEditButtonAction(ActionEvent actionEvent) throws IOException, LineUnavailableException, UnsupportedAudioFileException {
        ...
        cust.setEmail(c_email);
        cust.setAcc_no(c_account);
        cust.setAge(c_age);
        cust.setNid(c_nid);

        // getting id
        String id = LoggedUser.getCustomer_id();
        ...
        if (!cust.getAcc_no().isEmpty()) {
            cust.editCustomerAccountField(id,"account_number", cust.getAcc_no());
            LoggedUser.getCust().setAcc_no(cust.getAcc_no());
        }

        if (!c_curpass.isEmpty() && !c_newpass.isEmpty()) {
            if(cust.changePassword(id, c_curpass, c_newpass)) {
                SendMail.sendEmail("Password Changed","Your password has been changed successfully. " +
                        "If this was not done by you, immediately contact us at" +
                        " chondromollikaahmed@gmail.com", LoggedUser.getCust().getEmail());
            }
            else {
        ...
        String msg = "Your account details have been changed successfully. " +
                "If this was not done by you, then please contact us at" +
